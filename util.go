...
func (t *Twilio) GenerateSignature(url string, form url.Values) ([]byte, error) {
...
// header is an error.
func (t *Twilio) CheckRequestSignature(r *http.Request, baseURL string) (bool, error) {
	if r.Method != "POST" {
	...
	expected, err := t.GenerateSignature(url, r.PostForm)
	if err != nil {
		return false, err
	}
